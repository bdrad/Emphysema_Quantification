...
def spindlyness_index(region_mask):
    '''
    can be thought of a relatively measure of how thin/spindly a region is
    '''
...
def remove_borders(raw_im):
    '''
    Input: a 2D axial slice windowed according to the lung window level and width
    ...
    def remove_borders_second_pass(binary):
        h, w = binary.shape
        binary = remove_small_objects(binary, min_size=TRACHEA_REMOVAL_AREA*(w/512)**2) # default settings are good enough
    ...
    binary = remove_borders_second_pass(binary)
    binary = normal_fill_border_mask(binary)
    return binary


def get_segmented_lungs_trial_and_error(raw_im, plot=False):
    ...
    INPUT: output of remove_borders
    ...
    def label_and_select_regions(img):
        ...
        if i == -1:
            ...
            # cannot find a second valid region, return all regions that are not fragment/trachea
            # here fragment is broadly referred to small regions that is not the lung nor the airway (such as bowel)
    ...
    '''
    we also need to remove the lowest region in each side if that is a trachea/fragment region
    NOTE: this does not always work since sometimes the bowel appears at a position such that
    the following operation fails to remove the bowel
    For improvement, perhaps set a new condition such that if one region does not have overlap 
    with the other region in terms of x coordinates, then the higher region should be removed
    '''
    if len(label_1_regions) == 1 and len(label_2_regions) == 1: 
        ...
        if invalid_region_cond(label_2_regions[0], label_1_regions[0]):
            for coordinates in label_1_regions[0].coords:
                label_image[coordinates[0], coordinates[1]] = 0
        elif invalid_region_cond(label_1_regions[0], label_2_regions[0]):
            for coordinates in label_2_regions[0].coords:
                label_image[coordinates[0], coordinates[1]] = 0
...
def postprocess_remove_airway(lung_ct_2d, lung_mask_2d, airway_thres=-800):
    ...
    for j in candidate_js:
        ...
        if old_area/new_area >= PERCENT_OVERLAP: #this means the original region is one intact piece so should be airway
            ...
            if overlap_area/new_area > PERCENT_OVERLAP and overlap_area/np.sum(label_lung_mask_2d==best_i) > PERCENT_OVERLAP:
                # if the percent_overlap is reach, the original region is isolated and can be directly removed
    ...
    for j in range(1, np.max(label_new_binary)):
        ...
        new_lung_mask_2d[new_mask_j] = old_mask[old_mask_dict[best_i]][0] 
...
def get_3d_segmented_lung(lung_ct, fn=get_segmented_lungs_trial_and_error): # use axial
    '''
    INPUT: original 3d lung ct scan
    PIPELINE:
    ...
    for i in range(d):
        binarized_ct[i,...] = remove_borders(lung_ct[i,...]) # remove borders and binarize lung_ct
...
def separate_into_two_lungs(original_mask):
    ...
    def get_height_at_each_width_new(region_mask): 
        '''
        ...
        for j in range(region_mask.shape[1]):
            if j not in ys:
                ...
                '''
                i = min(xs[indices])
                while i+1 in xs[indices]:
                    i+=1
                height_at_each_width.append(i-min(xs[indices])+1)
                '''
                height_at_each_width.append(max(xs[indices])-min(xs[indices])+1)
        return height_at_each_width
    ...
    for i in range(d//10, d): # usually just the middle slices require this operation
        ...
        #print(i, np.sum(new_mask[i]==1), np.sum(new_mask[i]==2), np.all(ys < w/2),np.all((ys >= w/2)))
        ...
        # one label, with segmentation in both halves of the image, and total number of areas is <= 2
            ...
            if start2 == w:
                ...
                new_end = end1-diff//4
                #print(i,start1, end1, new_start, new_end)
...
def wrapper_fn(DICOM_dir, thres=-950):
    # test 3x3x3 and 3x3 kernels
    ...
    '''
    can modify below to get your patch average with desired kernel
    '''
    
    n = len(mask)
    binary_mask = mask > 0
    patched_avgs_3D = create_thresholding_3d_faster(lung_ct, binary_mask, patch_size=3)
    patched_avgs_2D = create_thresholding_2d_faster(lung_ct, binary_mask, patch_size=3)
    emphy_mask_3D = np.logical_and(patched_avgs_3D < thres, binary_mask)
    emphy_mask_2D = np.logical_and(patched_avgs_2D < thres, binary_mask)

    '''
    only take the first 3/4 axials slices because the inferior slices contain little area of the lung
    and sometimes the bowel is not removed 
    '''
    score_3D = np.sum(emphy_mask_3D[:n*3//4])/np.sum(binary_mask[:n*3//4])   
    score_2D = np.sum(emphy_mask_2D[:n*3//4])/np.sum(binary_mask[:n*3//4])    
         
    return lung_ct, mask, emphy_mask_3D, emphy_mask_2D, score_3D, score_2D
